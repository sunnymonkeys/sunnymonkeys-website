...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_subscription') {
        ...
        $sub = $existing->fetch();
        if ($sub) {
            $pdo->prepare('UPDATE subscriptions SET visible=?,base_name=?,base_price=?,billing_cycle=?,contract_start=?,notes=? WHERE client_id=?')
               ->execute([$visible,$base_name,$base_price,$billing_cycle,$contract_start,$notes,$id]);
        } else {
            $pdo->prepare('INSERT INTO subscriptions (client_id,visible,base_name,base_price,billing_cycle,contract_start,notes) VALUES (?,?,?,?,?,?,?)')
               ->execute([$id,$visible,$base_name,$base_price,$billing_cycle,$contract_start,$notes]);
        }
    ...
    elseif ($action === 'add_addon') {
        ...
        if ($name && $sub_id) {
            $pdo->prepare('INSERT INTO subscription_addons (subscription_id,name,price,promo_price,promo_cycles_remaining) VALUES (?,?,?,?,?)')
               ->execute([$sub_id,$name,$price,$promo_price,$promo_cycles]);
    ...
    elseif ($action === 'update_addon') {
        ...
        $pdo->prepare('UPDATE subscription_addons SET name=?,price=?,promo_price=?,promo_cycles_remaining=?,active=? WHERE id=?')
           ->execute([$name,$price,$promo_price,$promo_cycles,$active,$addon_id]);
        $msg = 'Add-on updated.';
        $redirect_tab = 'subscription';
    }
    elseif ($action === 'delete_addon') {
        $addon_id = (int)($_POST['addon_id'] ?? 0);
        $pdo->prepare('DELETE FROM subscription_addons WHERE id=?')->execute([$addon_id]);
        $msg = 'Add-on removed.';
        $redirect_tab = 'subscription';
    }
    elseif ($action === 'respond_request') {
        ...
        $pdo->prepare('UPDATE subscription_requests SET status=?,admin_note=? WHERE id=?')
           ->execute([$status,$note,$req_id]);
        $msg = 'Request updated.';
        $redirect_tab = 'requests';
    }

    header('Location: client-view.php?id='.$id.'&tab='.urlencode($redirect_tab).'&msg='.urlencode($msg));
    exit;
}
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  input[type=text], input[type=email], input[type=number], input[type=date], select, textarea {
    width: 100%; background: #111; border: 1px solid #2e2e2e; border-radius: 8px; padding: 10px 12px; color: #fff; font-size: 0.9rem; outline: none; font-family: inherit;
  }
...
<div class="container">
    ...
    <form method="POST">
      ...
      <div class="toggle-row">
        ...
        <div class="toggle-label">
          ...
          <span>When off, the "My Plan" section is hidden from their dashboard</span>
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Plan Name</label>
          <input type="text" name="base_name" value="<?= htmlspecialchars($sub['base_name'] ?? 'Base Retainer') ?>">
        </div>
        <div class="form-group">
          <label>Base Price ($)</label>
          <input type="number" name="base_price" step="0.01" value="<?= htmlspecialchars($sub['base_price'] ?? '1725.00') ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="date" name="contract_start" value="<?= htmlspecialchars($sub['contract_start'] ?? '') ?>">
        </div>
      </div>
      <div class="form-group">
  ...
  <div class="section-card">
    ...
    <table class="addon-table">
      <thead><tr>
        <th>Service</th><th>Regular</th><th>Promo Price</th><th>Promo Cycles Left</th><th>Status</th><th>Actions</th>
      </tr></thead>
      <tbody>
      ...
      <tr>
        ...
        <td>
          <?php if ($addon['promo_price'] !== null && $addon['promo_cycles_remaining'] > 0): ?>
        ...
        <td style="white-space:nowrap">
          ...
          <form method="POST" style="display:inline" onsubmit="return confirm('Remove this add-on?')">
      ...
      <tr id="edit-<?= $addon['id'] ?>" style="display:none">
        <td colspan="6" style="padding:14px 12px;background:#0a0a0a">
          <form method="POST" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr auto;gap:10px;align-items:end">
            ...
            <div><label>Status</label>
              <select name="active">
                <option value="1" <?= $addon['active'] ? 'selected':'' ?>>Active</option>
                <option value="0" <?= !$addon['active'] ? 'selected':'' ?>>Inactive</option>
              </select>
            </div>
    ...
    <hr class="divider">
    <?php else: ?>
    <p class="empty" style="margin-bottom:16px">No add-ons yet.</p>
    <?php endif; ?>
    <h3 style="font-size:0.88rem;margin-bottom:14px">Add New Add-on</h3>
    <form method="POST" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr auto;gap:12px;align-items:end">
      ...
      <input type="hidden" name="tab" value="subscription">
      <div><label>Service Name</label><input type="text" name="addon_name" placeholder="e.g. Email Monthly Tier" required></div>
      <div><label>Regular Price ($)</label><input type="number" name="addon_price" step="0.01" placeholder="225.00" required></div>
      <div><label>Promo Price ($)</label><input type="number" name="addon_promo_price" step="0.01" placeholder="0=FREE, blank=none"></div>
  ...
  <?php if (empty($all_requests)): ?>
    <p class="empty">No requests from this client yet.</p>
    ...
    <div class="request-card">
      ...
      <?php if (!empty($req['admin_note'])): ?>
        <div style="font-size:0.82rem;color:#666;padding-top:10px;border-top:1px solid #1e1e1e;margin-top:8px">
          <strong style="color:#555">Your note:</strong> <?= htmlspecialchars($req['admin_note']) ?>
        </div>
      <?php endif; ?>
      ...
      <form method="POST" style="display:flex;gap:10px;margin-top:14px;align-items:flex-end;flex-wrap:wrap">
        ...
        <div style="flex:1;min-width:200px">
          <label>Note to attach (optional)</label>
          <input type="text" name="admin_note" placeholder="Internal note...">
        </div>
